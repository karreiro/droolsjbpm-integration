...
public class SpecManagementServiceImplTest extends AbstractServiceImplTest {
    ...
    public void testUpdateContainerProcessConfig() {
        ...
        doReturn(configs).when(containerSpec).getConfigs();
        doReturn(expectedContainers).when(kieServerInstanceManager).startContainer(serverTemplate, containerSpec);

        final List<Container> actualContainers = specManagementService.updateContainerProcessConfig(processConfig,
                                                                                                    capability,
                                                                                                    serverTemplate,
                                                                                                    containerSpec);
        verify(configs).put(capability, processConfig);
        verify(containerSpec).setStatus(KieContainerStatus.STARTED);
        verify(kieServerInstanceManager).stopContainer(serverTemplate, containerSpec);
